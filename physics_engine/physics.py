...
def calculate_dst_to_player(x: int, y: int, player) -> float | int:
...
def normalize_angle_0_center(angle):
    return (angle + pi) % (2 * pi) - pi

def distance_between(x1, y1, x2, y2):
    dx, dy = get_dpos((x1, y1), (x2, y2))
    return math.sqrt(dx**2 + dy ** 2)

def get_dpos(pos1, pos2):
    """ Returns difference beetwen 2 positions"""
...
def get_theta_angle(dpos, player_angle):
    dx, dy = dpos
    player_angle = normalize_angle_0_center(player_angle)
    angle2 = normalize_angle_0_center(math.atan2(dy, dx))
    theta = normalize_angle_0_center(player_angle - angle2)
    if -1 < theta < 1: # Problème du symbole opposé
        # Fix rapide mais on peut faire plus joli niveau maths
        theta = pi/2 # On le met à 90 degré du joueur, il ne pourra pas le voir
